        ...
        <div class="flex flex-col justify-between gap-6 sm:items-center sm:flex-row">
          <div>
              <div class="text-xl font-medium text-dark">
                Daftar Kategori
                <p class="text-sm font-extralight text-slate-400 italic">Tentukan Kategori terbaik anda</p>
              </div>
              <!-- <p class="text-grey">Your team powers</p> -->
          </div>
          <NuxtLink to="/brands/add" class="btn btn-primary">Tambah Kategori</NuxtLink>
        </div>
      </div>
    </section>
...
onMounted(async () => {
  const dataGrid = new Grid({
      ...
      { 
        ...
        formatter: (cell, row) => {
          return h('button', {
            ...
            onClick: () => {
                router.push({ path: `/categories/edit/` + row.cells[0].data })
            }
          }, 'Edit')
        }
      }
    ...
    server: {
      url: 'http://127.0.0.1:3333/api/category',
