<template>
  <section class="py-[20px] w-full items-center justify-center px-4">
    <div v-if="showAlert" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
      ...
      <span class="block sm:inline">Category change successfully.</span>
      ...
      <div class="flex flex-wrap">
        <div class="w-full lg:w-1/3 mt-3 lg:mt-0">
          <div class="form-group px-2">
            ...
            <input v-model="category.name" type="text" class="input-field">
          </div>
        </div>
      </div>
...
onMounted(async () => {
  const response = await fetch(`http://127.0.0.1:3333/api/category/${route.params.id}`, {
    method: 'GET',
    headers: {
...
const updateData = async () => {
  const { status } = await useFetch(`http://127.0.0.1:3333/api/category/${route.params.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${cookie.value}`
