        ...
        <div class="flex flex-col justify-between gap-6 sm:items-center sm:flex-row">
          <div>
              <div class="text-xl font-medium text-dark">
                Daftar Permissions
                <p class="text-sm font-extralight text-slate-400 italic">Module di tangan anda</p>
              </div>
              <!-- <p class="text-grey">Your team powers</p> -->
          </div>
          <NuxtLink to="/brands/add" class="btn btn-primary">Tambah Permissions</NuxtLink>
        </div>
      </div>
    </section>
...
onMounted(async () => {
  const dataGrid = new Grid({
    ...
    fixedHeader: true,
    resizable: true,
    columns: [{ name: 'id', hidden:true }, 'Name', 'Slug', 
      { 
        name: 'Actions',
        formatter: (cell, row) => {
          return h('button', {
            ...
            onClick: () => {
                router.push({ path: `/permissions/edit/` + row.cells[0].data })
            }
          }, 'Edit')
        }
      }
    ...
    server: {
      url: 'http://127.0.0.1:3333/api/permissions',
