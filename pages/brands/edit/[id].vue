      ...
      <div class="flex flex-wrap">
        <div class="w-full lg:w-1/3 mt-3 lg:mt-0">
          <div class="form-group px-2">
            ...
            <input v-model="unit.name" type="text" class="input-field">
          </div>
        </div>
      </div>
...
onMounted(async () => {
  const response = await fetch(`http://127.0.0.1:3333/api/brands/${route.params.id}`, {
    method: 'GET',
    headers: {
  ...
  if (response.status === 201) {
    ...
    unit.name = data.data[0].name
  }
})


const updateData = async () => {
  const { status } = await useFetch(`http://127.0.0.1:3333/api/brands/${route.params.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${cookie.value}`
